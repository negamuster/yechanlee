...
export const SOURCES = [
  ...
  { id: 'chosunbiz', publisher: '조선비즈', region: 'kr', url: 'https://biz.chosun.com/arc/outboundfeeds/rss/?outputType=xml', hosts: ['biz.chosun.com'], imageHosts: ['images.chosun.com'], filterTopics: true },
...
export function parseFeed(xml, source, now = Date.now()) {
  ...
  return entries.slice(0, 150).flatMap(entry => {
    ...
    return [{ id: url, title, article_url: url, published_utc: new Date(timestamp).toISOString(), publisher: source.publisher, region: source.region, ...feedImage(entry, source) }]
  })
}
...
export async function collectNews(fetcher = fetch, sources = SOURCES, now = Date.now(), timeoutMs = 10000) {
  ...
  return {
    version: 'rss-node-v3', items: bounded, fetchedAt: now,
