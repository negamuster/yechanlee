...
const CIKS = new Set(['0001067983','0002012383','0000102909','0000093751','0001350694','0001423053','0001037389','0001536411','0002026053','0001656456','0001029160','0001697748','0001709323','0001649339'])
...
export function submissionRecords(data) {
  return array(data.form).flatMap((form, i) => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(period || '') || !/^\d{10}-\d{2}-\d{6}$/.test(accession || '') || !primary || !/^\d{4}-\d{2}-\d{2}$/.test(filedAt || '')) throw new Error('invalid_filing_metadata')
    return [{ form, period, accession, filedAt, primary, acceptedAt: data.acceptanceDateTime?.[i] || filedAt }]
  })
}
