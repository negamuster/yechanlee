...
const sections = [
  {
    ...
    subtitle: '경제 성장',
    desc: 'GDP는 일정 기간 동안 한 국가에서 생산된 모든 재화와 서비스의 시장 가치를 합산한 지표로, 경제 규모와 성장 속도를 측정하는 가장 대표적인 척도입니다. GDP 성장률은 경기 확장과 수축을 판단하는 기준이 되며, 두 분기 연속 마이너스 성장은 기술적 경기침체로 정의됩니다. 데이터를 볼 때는 절대 수치보다 전분기 대비 성장률의 방향과 속도에 주목하는 것이 중요하며, 시장 예상치 대비 상회 혹은 하회 여부가 자산 가격에 즉각적인 영향을 미칩니다.',
    links: [
  ...
  },
  {
    title: 'Labor Market',
    subtitle: '고용',
    desc: '고용 시장은 경제의 건강 상태를 보여주는 핵심 지표 중 하나입니다. 실업률은 경제활동인구 중 일자리를 찾고 있는 비율을 나타내며, 비농업 고용지수(NFP)는 농업을 제외한 전 산업에서 한 달간 새로 창출된 일자리 수를 보여줍니다. 또한 매주 발표되는 신규 실업수당 청구건수는 고용 시장의 변화를 가장 빠르게 반영하는 고빈도 지표로, 주간 단위로 고용 흐름을 점검하는 데 유용합니다. 숫자 자체보다 시장 예상치 대비 결과와 전월 수치의 수정 여부를 함께 확인하는 것이 중요합니다.',
    links: [
  ...
  },
  {
    ...
    subtitle: '소비지출',
    desc: '미국 경제에서 소비는 GDP의 약 70%를 차지하는 핵심 성장 동력입니다. 소매판매는 소비자들의 실제 지출 규모를 월간 단위로 보여주는 지표로, 경기 강도를 직접적으로 반영합니다. 소매판매가 예상을 상회하면 소비 심리가 강하고 경기가 견조하다는 신호로, 하회하면 소비 둔화 우려가 커지는 신호로 해석됩니다. 전체 소매판매와 함께 자동차를 제외한 근원 소매판매를 함께 보면 일시적 변동 요인을 걸러내고 소비 트렌드를 더 명확하게 파악할 수 있습니다.',
    links: [
