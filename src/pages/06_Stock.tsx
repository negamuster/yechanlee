...
export default function Stock() {
  ...
  const loadData = async () => {
    ...
    try {
      const [detRes, prevRes, newsRes, relRes, finRes, yrRes] = await Promise.allSettled([
      ...
      if (prevRes.status === 'fulfilled' && prevRes.value?.results?.[0]) {
        setPrevDay(prevRes.value.results[0])
      }
