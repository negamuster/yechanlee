...
const sections = [
  ...
  },
  {
    ...
    subtitle: '금리 정책',
    desc: '정책금리는 은행 간 초단기 자금 거래에 적용되는 기준 금리로, 금융 시스템 전반의 금리 구조를 형성하는 출발점입니다. FOMC는 인플레이션, 고용, 경제 성장 등 주요 거시지표를 바탕으로 금리 수준을 결정하며, 이는 경기 과열 억제 또는 경기 부양을 위한 핵심 수단으로 작용합니다. 금리 인상은 자금 조달 비용을 높여 수요를 억제하고, 금리 인하는 유동성을 확대해 투자와 소비를 자극하는 방향으로 작용합니다.',
  ...
  },
  {
    title: 'Policy Tools',
    subtitle: '통화정책 수단',
    desc: '중앙은행은 정책금리를 실제 시장에 전달하기 위해 다양한 수단을 활용합니다. 공개시장조작은 국채 매입·매도를 통해 시중 유동성을 직접 조절하는 핵심 수단이며, 재할인율 정책은 은행이 중앙은행으로부터 자금을 빌릴 때 적용되는 금리로 금융 시스템의 안전판 역할을 합니다. 지급준비율은 은행의 대출 가능 규모를 제한함으로써 신용 창출을 조절하는 수단입니다. 이러한 도구들은 함께 작동하며 금리 수준과 자금 흐름을 시장 전반에 전달합니다.',
    link: null,
  },
]
